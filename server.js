...
function newComment(url, request) {
  const response = {}
  // this will filter out the request with no body
  const newComment = request.body && request.body.comment;

  if (newComment && newComment.body && newComment.username && 
      newComment.articleId && database.users[newComment.username] &&
...
function deleteComment(url, request) {
  ...
  if (existingComment) {
    let userPos = database.users[existingComment.username].commentIds.indexOf(id);
    database.users[existingComment.username].commentIds.splice(userPos, 1);
    let articlePos = database.articles[existingComment.articleId].commentIds.indexOf(id);
    database.articles[existingComment.articleId].commentIds.splice(articlePos, 1);
...
function upvoteComment(url, request) {
  ...
  if (username && database.users[username] && database.comments[id]) {
    if(database.comments[id].upvotedBy.indexOf(username) === -1 ) {
      database.comments[id].upvotedBy.push(username);
      let pos = database.comments[id].downvotedBy.indexOf(username);
      if (pos !== -1) {
...
//this is pretty much the same as upvote...
function downvoteComment(url, request) {
  ...
  if (username && database.users[username] && database.comments[id]) {
    if(database.comments[id].downvotedBy.indexOf(username) === -1 ) {
      database.comments[id].downvotedBy.push(username);
      let pos = database.comments[id].upvotedBy.indexOf(username);
      if (pos !== -1) {
